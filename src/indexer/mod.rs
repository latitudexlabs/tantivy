...
mod tests_mmap {
    ...
    #[test]
    fn test_edge_ngrams_with_all_combinations() -> crate::Result<()> {
        // Test edge ngram generation with all combinations mode
        let mut schema_builder = Schema::builder();
        ...
        let text_indexing = TextFieldIndexing::default()
            ...
            .set_word_ngrams(
                WordNgramConfig::with_set(WordNgramSet::new().with_ngram_ff())
                    ...
                    .with_all_combinations_window_size(10)
                    .with_edge_ngram()
                    .with_min_edge_ngram(2),
            );
        ...
        // Verify original terms are indexed
        let term_query = TermQuery::new(
            Term::from_field_text(text_field, "test1"),
            IndexRecordOption::Basic,
        );
        let top_docs = searcher.search(&term_query, &TopDocs::with_limit(10).order_by_score())?;
        assert_eq!(top_docs.len(), 1, "Full term 'test1' should be indexed");

        // Verify edge ngram bigrams are indexed
        let bigram_checks = vec![
            ("test1 xyz", "Full term 'test1' with full term 'xyz'"),
            ("test1 xy", "Full term 'test1' with edge ngram 'xy'"),
            ("test xyz", "Edge ngram 'test' with full term 'xyz'"),
            ("test xy", "Edge ngram 'test' with edge ngram 'xy'"),
            ("tes xyz", "Edge ngram 'tes' with full term 'xyz'"),
            ("tes xy", "Edge ngram 'tes' with edge ngram 'xy'"),
            ("te xyz", "Edge ngram 'te' with full term 'xyz'"),
            ("te xy", "Edge ngram 'te' with edge ngram 'xy'"),
        ];

        for (bigram, description) in bigram_checks {
            let bigram_query = TermQuery::new(
                Term::from_field_text(text_field, bigram),
                IndexRecordOption::Basic,
            );
            let top_docs =
                searcher.search(&bigram_query, &TopDocs::with_limit(10).order_by_score())?;
            assert_eq!(
                top_docs.len(),
                1,
                "Bigram '{}' should be indexed ({})",
                bigram,
                description
            );
        }
    ...
    fn test_edge_ngrams_min_length() -> crate::Result<()> {
        ...
        let text_indexing = TextFieldIndexing::default()
            ...
            .set_word_ngrams(
                WordNgramConfig::with_set(WordNgramSet::new().with_ngram_ff())
                    .with_all_combinations()
                    .with_edge_ngram()
                    .with_min_edge_ngram(3), // Min length 3
            );
        ...
        // Index "hello world"
        // With min_edge_ngram=3, "hello" generates: hello, hell, hel (stops at 3 chars)
        // "world" generates: world, worl, wor
        index_writer.add_document(doc!(
        ...
        // Should have edge ngrams down to 3 characters
        let bigram_query = TermQuery::new(
            Term::from_field_text(text_field, "hel wor"),
            IndexRecordOption::Basic,
        );
        let top_docs = searcher.search(&bigram_query, &TopDocs::with_limit(10).order_by_score())?;
        assert_eq!(
            top_docs.len(),
            1,
            "Edge ngram 'hel wor' (min length 3) should be indexed"
        );

        // Should NOT have edge ngrams shorter than min_edge_ngram (2 characters)
        let short_bigram_query = TermQuery::new(
            Term::from_field_text(text_field, "he wo"),
            IndexRecordOption::Basic,
        );
        let top_docs =
            searcher.search(&short_bigram_query, &TopDocs::with_limit(10).order_by_score())?;
        assert_eq!(
            top_docs.len(),
            0,
            "Edge ngram 'he wo' (below min length) should NOT be indexed"
        );
    ...
    #[test]
    fn test_edge_ngrams_with_consecutive_ngrams() -> crate::Result<()> {
        // Test edge ngrams with consecutive ngrams (not all combinations)
        let mut schema_builder = Schema::builder();

        let text_indexing = TextFieldIndexing::default()
            .set_index_option(IndexRecordOption::WithFreqsAndPositions)
            .set_word_ngrams(
                WordNgramConfig::with_set(WordNgramSet::new().with_ngram_ff().with_ngram_fr().with_ngram_rf())
                    .with_all_combinations()
                    .with_all_combinations_window_size(20)
                    .with_edge_ngram()
        ...
        let mut index_writer = index.writer(15_000_000)?;
         ...
         index_writer.add_document(doc!(
            text_field => "123 main street new city york 12080"
        ))?;
        ...
        let terms = vec![
            Term::from_field_text(text_field, "new"),
            Term::from_field_text(text_field, "york"),
            Term::from_field_text(text_field, "new"),
            Term::from_field_text(text_field, "york"),
        ];

        // In consecutive mode, should have consecutive bigrams
        let bigram_query = crate::query::FuzzyPhraseQuery::new(terms, 1);
        let top_docs = searcher.search(&bigram_query, &TopDocs::with_limit(10).order_by_score())?;
        assert!(
            top_docs.len() >= 1,
            "Consecutive edge ngram bigram should be indexed"
        );
    ...
    #[test]
    fn test_edge_ngrams_builder_api() {
        // Test the builder API for edge ngrams
        ...
        assert!(config.edge_ngram, "edge_ngram should be enabled");
        assert_eq!(config.min_edge_ngram, 3, "min_edge_ngram should be 3");
        assert!(
            config.all_combinations,
            "all_combinations should be enabled"
        );
    }

    #[test]
    fn test_edge_ngrams_fluent_api() {
        // Test the fluent API for edge ngrams
        ...
        assert!(config.edge_ngram, "edge_ngram should be enabled");
        assert_eq!(config.min_edge_ngram, 2, "min_edge_ngram should be 2");
    }

    #[test]
    fn test_identical_word_ngrams_filtered() -> crate::Result<()> {
        // Test that bigrams/trigrams with identical words are NOT indexed
        let mut schema_builder = Schema::builder();
        ...
        let text_indexing = TextFieldIndexing::default()
            ...
            .set_word_ngrams(
                WordNgramConfig::with_set(
                    WordNgramSet::new().with_ngram_ff().with_ngram_fff(),
                )
                .with_all_combinations()
                .with_edge_ngram()
                .with_min_edge_ngram(2),
        ...
        // Index text with repeated words that could create identical-word ngrams
        index_writer.add_document(doc!(
            text_field => "new york new"
        ))?;
        ...
        // Verify that "new new" bigram is NOT indexed
        let identical_bigram_query = TermQuery::new(
            Term::from_field_text(text_field, "new new"),
            IndexRecordOption::Basic,
        );
        let top_docs =
            searcher.search(&identical_bigram_query, &TopDocs::with_limit(10).order_by_score())?;
        assert_eq!(
            top_docs.len(),
            0,
            "Identical-word bigram 'new new' should NOT be indexed"
        );

        // Verify that "new new new" trigram is NOT indexed
        let identical_trigram_query = TermQuery::new(
            Term::from_field_text(text_field, "new new new"),
            IndexRecordOption::Basic,
        );
        let top_docs = searcher.search(
            &identical_trigram_query,
            &TopDocs::with_limit(10).order_by_score(),
        )?;
        assert_eq!(
            top_docs.len(),
            0,
            "Identical-word trigram 'new new new' should NOT be indexed"
        );

        // Verify that different-word bigrams ARE still indexed
        let valid_bigram_query = TermQuery::new(
            Term::from_field_text(text_field, "new york"),
            IndexRecordOption::Basic,
        );
        let top_docs =
            searcher.search(&valid_bigram_query, &TopDocs::with_limit(10).order_by_score())?;
        assert_eq!(
            top_docs.len(),
            1,
            "Different-word bigram 'new york' should be indexed"
        );
