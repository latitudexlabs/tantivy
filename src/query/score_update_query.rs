...
pub type ScoreUpdateFn = Option<Arc<dyn Fn(Score) -> Score + Send + Sync>>;
...
pub type ScoreUpdateWithContextFn =
    Option<Arc<dyn Fn(Option<u64>, Option<u64>, Score, Option<u8>) -> Score + Send + Sync>>;
...
impl ScoreUpdateQueryWithContext {
    ...
    /// * `score_update_fn` - Optional function that takes field value, base value, score, and sort order. If None, returns original score.
    pub fn new(
        query: Box<dyn Query>,
        scoring_context: ScoringContext,
        score_update_fn: ScoreUpdateWithContextFn,
    ...
    {
        ScoreUpdateQueryWithContext::new(query, scoring_context, Some(Arc::new(f)))
    }
}
...
impl ScoreUpdateQuery {
    ...
    ///
    /// * `query` - The underlying query to wrap
    /// * `score_update_fn` - Optional function that takes the original score and returns the updated score. If None, returns original score.
    ...
    {
        ScoreUpdateQuery::new(query, Some(Arc::new(f)))
    }
}
...
impl Weight for ScoreUpdateWeight {
    ...
    fn explain(&self, reader: &SegmentReader, doc: u32) -> crate::Result<Explanation> {
        ...
        let updated_score = if let Some(ref update_fn) = self.score_update_fn {
            update_fn(original_score)
        } else {
            original_score
        };
...
impl Scorer for ScoreUpdateScorer {
    ...
    fn score(&mut self) -> Score {
        ...
        if let Some(ref update_fn) = self.score_update_fn {
            update_fn(original_score)
        } else {
            original_score
        }
...
impl Weight for ScoreUpdateWeightWithContext {
    fn scorer(&self, reader: &SegmentReader, boost: Score) -> crate::Result<Box<dyn Scorer>> {
        ...
        let field_name = reader
            .schema()
            .get_field_name(self.scoring_context.scoring_field);
...
mod tests {
    ...
    fn test_score_update_query_basic() -> crate::Result<()> {
        ...
        let query = ScoreUpdateQuery::new(Box::new(AllQuery), Some(Arc::new(|score| score * 2.0)));
    ...
    fn test_score_update_query_logarithmic() -> crate::Result<()> {
        ...
        let query = ScoreUpdateQuery::with_fn(Box::new(AllQuery), |score| (1.0 + score).ln());
    ...
    fn test_score_update_query_with_context() -> crate::Result<()> {
        ...
        let context = ScoringContext::new(popularity_field).with_base_value(100);
    ...
    #[test]
    fn test_score_update_query_none_returns_original() -> crate::Result<()> {
        let schema = Schema::builder().build();
        let index = Index::create_in_ram(schema);
        let mut index_writer: IndexWriter = index.writer_for_tests()?;
        index_writer.add_document(TantivyDocument::new())?;
        ...
        // Create a query with None function - should return original score
        let query = ScoreUpdateQuery::new(Box::new(AllQuery), None);

        let explanation = query.explain(&searcher, DocAddress::new(0, 0u32))?;
        // AllQuery returns score of 1.0, with None function it should remain 1.0
    ...
    #[test]
    fn test_score_update_query_with_context_none_returns_original() -> crate::Result<()> {
        use super::{ScoreUpdateQueryWithContext, ScoringContext};
        use crate::schema::FAST;
        ...
        let mut doc = TantivyDocument::new();
        doc.add_u64(popularity_field, 100);
        index_writer.add_document(doc)?;
        ...
        // Create a scoring context with None function
        let context = ScoringContext::new(popularity_field).with_base_value(100);

        let query = ScoreUpdateQueryWithContext::new(Box::new(AllQuery), context, None);
